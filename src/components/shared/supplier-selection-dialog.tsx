

'use client';

import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { usePos } from '@/contexts/pos-context';
import type { Supplier } from '@/lib/types';
import { Input } from '@/components/ui/input';
import { Edit, UserPlus, Truck } from 'lucide-react';
import { AddSupplierDialog } from '@/app/management/suppliers/components/add-supplier-dialog';
import { EditSupplierDialog } from '@/app/management/suppliers/components/edit-supplier-dialog';
import { cn } from '@/lib/utils';
import { ScrollArea } from '@/components/ui/scroll-area';

interface SupplierSelectionDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSupplierSelected: (supplier: Supplier) => void;
}

export function SupplierSelectionDialog({ isOpen, onClose, onSupplierSelected }: SupplierSelectionDialogProps) {
  const { suppliers } = usePos();
  const [isAddSupplierOpen, setAddSupplierOpen] = useState(false);
  const [isEditSupplierOpen, setEditSupplierOpen] = useState(false);
  const [supplierToEdit, setSupplierToEdit] = useState<Supplier | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [highlightedIndex, setHighlightedIndex] = useState(-1);

  const itemRefs = useRef<(HTMLDivElement | null)[]>([]);
  const inputRef = useRef<HTMLInputElement>(null);

  const filteredSuppliers = useMemo(() => {
    if (!suppliers) return [];
    return suppliers.filter(supplier => {
        const lowerSearchTerm = searchTerm.toLowerCase();
        return (
            supplier.name.toLowerCase().includes(lowerSearchTerm) ||
            supplier.id.toLowerCase().includes(lowerSearchTerm) ||
            (supplier.postalCode && supplier.postalCode.toLowerCase().includes(lowerSearchTerm)) ||
            (supplier.city && supplier.city.toLowerCase().includes(lowerSearchTerm))
        );
    });
  }, [suppliers, searchTerm]);

  useEffect(() => {
    if (isOpen) {
      setSearchTerm('');
      setTimeout(() => inputRef.current?.focus(), 100);
    }
  }, [isOpen]);
  
  useEffect(() => {
    setHighlightedIndex(filteredSuppliers.length > 0 ? 0 : -1);
  }, [searchTerm, filteredSuppliers.length]);
  
  useEffect(() => {
    if (highlightedIndex !== -1 && itemRefs.current[highlightedIndex]) {
      itemRefs.current[highlightedIndex]?.scrollIntoView({
        behavior: 'smooth',
        block: 'nearest',
      });
    }
  }, [highlightedIndex]);

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setHighlightedIndex(prev => Math.min(prev + 1, filteredSuppliers.length - 1));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setHighlightedIndex(prev => Math.max(prev - 1, 0));
    } else if (e.key === 'Enter') {
      e.preventDefault();
      if (highlightedIndex >= 0 && filteredSuppliers[highlightedIndex]) {
        handleSelect(filteredSuppliers[highlightedIndex]);
      }
    }
  };

  const handleSelect = (supplier: Supplier) => {
    onSupplierSelected(supplier);
    onClose();
  };

  const handleEdit = () => {
    if (highlightedIndex >= 0 && filteredSuppliers[highlightedIndex]) {
      setSupplierToEdit(filteredSuppliers[highlightedIndex]);
      setEditSupplierOpen(true);
    }
  }

  const handleSupplierAdded = (newSupplier: Supplier) => {
    setAddSupplierOpen(false);
    onSupplierSelected(newSupplier);
    onClose();
  }
  
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-3xl h-[70vh] flex flex-col p-0">
          <DialogHeader className="p-6 pb-2">
            <DialogTitle>Choisir un fournisseur</DialogTitle>
             <DialogDescription>
                Recherchez par nom, code, code postal ou ville.
            </DialogDescription>
          </DialogHeader>
          <div className="px-6">
            <Input
              ref={inputRef}
              placeholder="Rechercher un fournisseur..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              onKeyDown={handleKeyDown}
            />
          </div>
          <div className="flex-1 min-h-0 px-6">
            <ScrollArea className="h-full">
              {filteredSuppliers.length === 0 ? (
                <div className="py-6 text-center text-sm">Aucun fournisseur trouvé.</div>
              ) : (
                <div className="space-y-1 py-2">
                  {filteredSuppliers.map((supplier, index) => (
                    <div
                      key={supplier.id}
                      ref={(el) => (itemRefs.current[index] = el)}
                      onClick={() => handleSelect(supplier)}
                      onMouseEnter={() => setHighlightedIndex(index)}
                      className={cn(
                        "flex items-center justify-between p-3 rounded-md cursor-pointer",
                        index === highlightedIndex ? "bg-accent text-accent-foreground" : "hover:bg-accent/50"
                      )}
                    >
                      <p className="font-semibold">
                        {supplier.name}
                        <span className="ml-4 text-xs text-muted-foreground font-normal">
                          {`(${supplier.id.slice(0, 8)}...) | ${supplier.postalCode || ''} ${supplier.city || ''}`}
                        </span>
                      </p>
                    </div>
                  ))}
                </div>
              )}
            </ScrollArea>
          </div>
          <DialogFooter className="p-4 border-t gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setAddSupplierOpen(true)}>
              <Truck className="mr-2 h-4 w-4" />
              Nouveau fournisseur
            </Button>
            <Button variant="secondary" onClick={handleEdit} disabled={highlightedIndex === -1}>
              <Edit className="mr-2 h-4 w-4" />
              Modifier
            </Button>
            <Button variant="ghost" onClick={onClose}>
              Annuler
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      
      <AddSupplierDialog isOpen={isAddSupplierOpen} onClose={() => setAddSupplierOpen(false)} onSupplierAdded={handleSupplierAdded} />
      <EditSupplierDialog isOpen={isEditSupplierOpen} onClose={() => setEditSupplierOpen(false)} supplier={supplierToEdit} />
    </>
  );
}
