...
export function CheckoutModal({ isOpen, onClose, totalAmount }: CheckoutModalProps) {
  const { clearOrder, recordSale, order, orderTotal, orderTax, paymentMethods, customers, currentSaleId, cameFromRestaurant, setCameFromRestaurant, currentSaleContext, user, paymentMethodImageOpacity, resetCommercialPage, addCheque, addDunningLog, updateSale } = usePos();
  ...
  const balanceDue = useMemo(() => displayTotalAmount - amountPaid, [displayTotalAmount, amountPaid]);
 ...
 const handleFinalizeSale = useCallback(async (finalPayments: Payment[]) => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        if (!isPaid) {
            setCurrentAmount(balanceDue > 0 ? balanceDue.toFixed(2) : '');
        }
  ...
  }, [isOpen, isPaid, totalAmount, balanceDue, customers, currentSaleContext]);
  ...
  const handleChequeChange = (index: number, field: keyof Omit<Cheque, 'id' | 'factureId' | 'clientId'>, value: any) => {
      setCheques(prev => {
        ...
        if (field === 'montant') {
            ...
            currentCheque[field] = parseFloat(Math.min(newAmount, maxAllowedForThisCheque).toFixed(2));
        } else {
 ...
 const handleConfirmCheques = () => {
    ...
    const chequePayments: Payment[] = cheques.map(cheque => ({
        method: checkPaymentMethod!,
        amount: parseFloat(cheque.montant.toFixed(2)),
        date: paymentDate as any,
    }));
    
    const newPayments = [...payments, ...chequePayments];
            ...
            {cheques.map((cheque, index) => (
              <Card key={index} className="p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                    <div className="space-y-1"><Label>Montant (€)</Label><Input type="number" value={cheque.montant.toFixed(2)} onChange={e => handleChequeChange(index, 'montant', e.target.value)} /></div>
