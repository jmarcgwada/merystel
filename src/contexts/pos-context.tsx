...
export function PosProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: items = [], isLoading: itemsLoading } = useCollection<Item>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'items'), [firestore, companyId]));
  const { data: categories = [], isLoading: categoriesLoading } = useCollection<Category>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'categories'), [firestore, companyId]));
  const { data: customers = [], isLoading: customersLoading } = useCollection<Customer>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'customers'), [firestore, companyId]));
  const { data: tables = [], isLoading: tablesLoading } = useCollection<Table>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'tables'), [firestore, companyId]));
  const { data: sales = [], isLoading: salesLoading } = useCollection<Sale>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'sales'), [firestore, companyId]));
  const { data: paymentMethods = [], isLoading: paymentMethodsLoading } = useCollection<PaymentMethod>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'paymentMethods'), [firestore, companyId]));
  const { data: vatRates = [], isLoading: vatRatesLoading } = useCollection<VatRate>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'vatRates'), [firestore, companyId]));
  const { data: heldOrders = [], isLoading: heldOrdersLoading } = useCollection<HeldOrder>(useMemoFirebase(() => companyId && collection(firestore, 'companies', companyId, 'heldOrders'), [firestore, companyId]));
  const { data: companyInfo, isLoading: companyInfoLoading } = useDoc<CompanyInfo>(useMemoFirebase(() => companyId && doc(firestore, 'companies', companyId), [firestore, companyId]));
  ...
  const updateEntity = useCallback(async (collectionName: string, id: string, data: any, toastTitle: string) => {
    try {
  ...
  const deleteEntity = useCallback(async (collectionName: string, id: string, toastTitle: string) => {
    try {
      const ref = getDocRef(collectionName, id);
      await deleteDoc(ref);
      toast({ title: toastTitle });
    ...
    setOrder(currentOrder =>
      currentOrder.map(item => {
        if (item.id === itemId) {
          ...
          return { ...item, discount: discountAmount, discountPercent, total: newTotal > 0 ? newTotal : 0 };
        }
        return item;
      })
    );
  ...
  const orderTotal = useMemo(() => order.reduce((sum, item) => sum + item.total, 0), [order]);
  ...
  const updateTableOrder = useCallback(async (tableId: string, orderData: OrderItem[]) => {
    try {
      const tableRef = getDocRef('tables', tableId);
      await setDoc(tableRef, { order: orderData, status: orderData.length > 0 ? 'occupied' : 'available' }, { merge: true });
    } catch (error) {
  ...
  const saveTableOrderAndExit = useCallback(async (tableId: string, orderData: OrderItem[]) => {
  ...
  const promoteTableToTicket = useCallback(async (tableId: string) => {
    ...
    const heldOrderRef = existingHeldOrder ? getDocRef('heldOrders', existingHeldOrder.id) : doc(getCollectionRef('heldOrders'));
  ...
  // #region Sales
  const recordSale = useCallback(async (saleData: Omit<Sale, 'id' | 'date' | 'ticketNumber'>) => {
    ...
    const finalSale: Omit<Sale, 'id'> = {
      ...
      ticketNumber,
      status: 'paid',
      ...(currentSaleContext?.tableId && { tableId: currentSaleContext.tableId, tableName: currentSaleContext.tableName }),
    };
    const newSaleRef = doc(getCollectionRef('sales'));
    batch.set(newSaleRef, finalSale);
    await batch.commit();
  }, [companyId, currentSaleId, heldOrders, firestore, getDocRef, sales, getCollectionRef, currentSaleContext]);
  // #endregion

  // #region Generic Entity Management
  const addCategory = useCallback((category: Omit<Category, 'id'>) => addEntity('categories', category, 'Catégorie ajoutée'), [addEntity]);
  const updateCategory = useCallback((category: Category) => updateEntity('categories', category.id, category, 'Catégorie modifiée'), [updateEntity]);
  const deleteCategory = useCallback((id: string) => deleteEntity('categories', id, 'Catégorie supprimée'), [deleteEntity]);
  const toggleCategoryFavorite = useCallback((id: string) => {
    if (!categories) return;
    const category = categories.find(c => c.id === id);
    if (category) updateEntity('categories', id, { isFavorite: !category.isFavorite }, 'Favori mis à jour');
  }, [categories, updateEntity]);

  const addItem = useCallback((item: Omit<Item, 'id'>) => addEntity('items', item, 'Article créé'), [addEntity]);
  const updateItem = useCallback((item: Item) => updateEntity('items', item.id, item, 'Article modifié'), [updateEntity]);
  const deleteItem = useCallback((id: string) => deleteEntity('items', id, 'Article supprimé'), [deleteEntity]);
  const toggleItemFavorite = useCallback((id: string) => {
    if (!items) return;
    const item = items.find(i => i.id === id);
    if (item) updateEntity('items', id, { isFavorite: !item.isFavorite }, 'Favori mis à jour');
  }, [items, updateEntity]);
  const toggleFavoriteForList = useCallback(async (itemIds: string[], setFavorite: boolean) => {
    const batch = writeBatch(firestore);
    itemIds.forEach(id => {
  ...
  const addCustomer = useCallback((customer: Omit<Customer, 'id' | 'isDefault'>) => {
    if (!customers) return;
    const newCustomer = { ...customer, isDefault: !customers.some(c => c.isDefault) };
    addEntity('customers', newCustomer, 'Client ajouté');
  }, [addEntity, customers]);
  const updateCustomer = useCallback((customer: Customer) => updateEntity('customers', customer.id, customer, 'Client modifié'), [updateEntity]);
  const deleteCustomer = useCallback((id: string) => deleteEntity('customers', id, 'Client supprimé'), [deleteEntity]);
  const setDefaultCustomer = useCallback(async (customerId: string) => {
    if (!customers) return;
    const batch = writeBatch(firestore);
    customers.forEach(c => {
  ...
  const addPaymentMethod = useCallback((method: Omit<PaymentMethod, 'id'>) => addEntity('paymentMethods', method, 'Moyen de paiement ajouté'), [addEntity]);
  const updatePaymentMethod = useCallback((method: PaymentMethod) => updateEntity('paymentMethods', method.id, method, 'Moyen de paiement modifié'), [updateEntity]);
  const deletePaymentMethod = useCallback((id: string) => deleteEntity('paymentMethods', id, 'Moyen de paiement supprimé'), [deleteEntity]);
  ...
  const updateVatRate = useCallback((vatRate: VatRate) => updateEntity('vatRates', vatRate.id, vatRate, 'Taux de TVA modifié'), [updateEntity]);
  const deleteVatRate = useCallback((id: string) => deleteEntity('vatRates', id, 'Taux de TVA supprimé'), [deleteEntity]);
  ...
  const popularItems = useMemo(() => {
    ...
    return Object.values(itemCounts).sort((a, b) => b.count - a.count).slice(0, popularItemsCount).map(i => i.item);
  }, [sales, items, popularItemsCount]);
  // #endregion

  const value = useMemo(
    () => ({
      order, setOrder, addToOrder, removeFromOrder, updateQuantity, updateQuantityFromKeypad, applyDiscount, clearOrder, orderTotal, orderTax, isKeypadOpen, setIsKeypadOpen, currentSaleId, setCurrentSaleId, currentSaleContext, recentlyAddedItemId, setRecentlyAddedItemId,
      items, addItem, updateItem, deleteItem, toggleItemFavorite, toggleFavoriteForList, popularItems,
      categories, addCategory, updateCategory, deleteCategory, toggleCategoryFavorite,
      customers, addCustomer, updateCustomer, deleteCustomer, setDefaultCustomer,
      tables, setTables: () => {}, addTable, updateTable, deleteTable, forceFreeTable, selectedTable, setSelectedTable, setSelectedTableById, updateTableOrder, saveTableOrderAndExit, promoteTableToTicket,
      sales, recordSale,
      paymentMethods, addPaymentMethod, updatePaymentMethod, deletePaymentMethod,
      vatRates, addVatRate, updateVatRate, deleteVatRate,
      heldOrders, holdOrder, recallOrder, deleteHeldOrder,
      showTicketImages, setShowTicketImages, popularItemsCount, setPopularItemsCount, itemCardOpacity, setItemCardOpacity, enableRestaurantCategoryFilter, setEnableRestaurantCategoryFilter,
    ...
    [
      order, setOrder, addToOrder, removeFromOrder, updateQuantity, updateQuantityFromKeypad, applyDiscount, clearOrder, orderTotal, orderTax, isKeypadOpen, currentSaleId, currentSaleContext, recentlyAddedItemId,
      items, addItem, updateItem, deleteItem, toggleItemFavorite, toggleFavoriteForList, popularItems,
      categories, addCategory, updateCategory, deleteCategory, toggleCategoryFavorite,
      customers, addCustomer, updateCustomer, deleteCustomer, setDefaultCustomer,
      tables, addTable, updateTable, deleteTable, forceFreeTable, selectedTable, setSelectedTable, setSelectedTableById, updateTableOrder, saveTableOrderAndExit, promoteTableToTicket,
      sales, recordSale,
      paymentMethods, addPaymentMethod, updatePaymentMethod, deletePaymentMethod,
      vatRates, addVatRate, updateVatRate, deleteVatRate,
      heldOrders, holdOrder, recallOrder, deleteHeldOrder,
